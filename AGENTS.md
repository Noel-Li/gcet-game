...
- **`game1_test.unity`** — the legacy region-access test scene. It has 198 scene roots, including its authored R1–R3 regions, three `InvisibleWall` gates, and test NPCs. Its YAML contains one unique object block per fileID and no dangling internal references; duplicate map/NPC blocks accidentally spliced in by the July merge were removed.
  ...
  - **Latest background resync:** `game1` now contains every authoritative `background.unity` root except its three obsolete Area roots. Sixteen superseded tree/wall/arch roots were removed and thirteen current arch/tree/bridge/lantern roots were imported; five new wall children were also added to the main `background` hierarchy. All common source `BoxCollider2D` fileIDs and serialized settings are synchronized except the intentional Trader override, and Guopinyu continues to replace the source collider with its game-only compact collider. Trader collider `151484826` is a foot-level local `2.4×1.5` box at offset `(1.715, 0.75)`; Guopinyu collider `3500000200` uses the same local size at offset `(1.665, 0.75)`. All character positions match the source. Other synchronized transform changes include the furniture table at `(22.67, 18.21)` and `Water Tiles_4 (8)` at `(41.4057, 20.9313)` with scale `(1.3645359, 5.5030994, 1)`. The 23 imported flower-cluster SpriteRenderers continue to use `WalkInFront`; gameplay-only Player/NPC controllers, conversations, persistent repeat-state handling, compact foot colliders, and both UI overlays remain intact.
...
- **`background.unity`** — the authoritative 191-root map source synchronized into `game1`; it includes the newest expanded environment, replacement tree/bridge/arch/lantern roots, Trader/Guopinyu content, furniture, flower clusters, large grass-patch roots, and the `motherMain_0` and `lilguyMain_0` NPC visuals.
...
`PlayerMovement.cs`, `PlayerWalkAnimation.cs`, `OpeningDialogue.cs`, `StartGame.cs`, `BackgroundMusic.cs`, `ComicSequence.cs`, `ControlsSceneEntry.cs`, `ControlsOverlayController.cs`, `ReviewBookController.cs`, `ReviewBookHintVisibility.cs`, `ReviewBookView.cs`, `GameArea.cs`, `GameRegion.cs`, `NpcController.cs`, `FollowCamera.cs`, `ColorSprite.cs`. All in `Assembly-CSharp` (no `.asmdef`). See README for the message flow.
