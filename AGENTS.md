...
- **`game1_test.unity`** — the legacy region-access test scene. It has 198 scene roots, including its authored R1–R3 regions, three `InvisibleWall` gates, and test NPCs. Its YAML contains one unique object block per fileID and no dangling internal references; duplicate map/NPC blocks accidentally spliced in by the July merge were removed.
  ...
  - **Latest background resync:** `game1` contains all 189 current authoritative `background.unity` roots after omitting the three obsolete Area helpers. The follow-up source revision removed the standalone `Tree_4` root; it also refreshed shared NPC/tree SpriteRenderers, moved `treesOne_0`, `treesOne_0 (3)`, `treesOne_0 (4)`, `treesTwo_0`, `treesTwo_0 (2)`, and `femNPCone_0`, and updated the `arch_0`, `Water Tiles_6 (6)`, and `Water Tiles_4 (8)` colliders. The current market/NPC roots remain synchronized, including `arch_0`, `riceFarmer_0`, both male NPCs, both female NPCs, `redbannerHouse_0`, `table_0`, `roastDuckStall (1)_0`, and `vegetableStall_0`. The game-only Player visual child, NPC controllers and conversations, review-book/controls/opening UI, region gates, and persistent tracing/repeat-state wiring remain intact. Two additional game-only wall roots, `TX Tileset Wall_8 (3)` and `TX Tileset Wall_8 (4)`, are also retained, bringing `game1` to 201 roots.
...
- **`background.unity`** — the authoritative 192-root map source synchronized into `game1`; it includes the expanded environment, current market/arch/furniture roots, four ambient male/female NPCs plus the rice farmer, Trader/Guopinyu content, flower clusters, large grass-patch roots, and the `motherMain_0` and `lilguyMain_0` NPC visuals.
