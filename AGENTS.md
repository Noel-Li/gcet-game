...
- **`game1.unity`** — the current playable scene. Its full visual map is synchronized from all 189 roots in `background.unity`, with two additional game-only UI roots (`Controls Hint Canvas` and `Opening Location Overlay`) for **191 scene roots total**. The synchronized map includes the 47-object `background` hierarchy, expanded standalone houses, trees, bridges, water, wall, grass/flower, pavement, and fence props, plus the newest three houses, extra tree, 23 flower clusters, 12 large grass patches, furniture table, `Trader`, `guopinyuMain_0`, `motherMain_0`, and `lilguyMain_0` roots. Gameplay roots retain the newer `game1` wiring (`PauseController`, `Player`, three Areas, `Soldier`, `Silk Seller`, `Main Camera`, and `EventSystem`); the obsolete game-only `Bridge02 (1)` visual is removed because it is absent from the authoritative background scene. The persistent music source is supplied by `StartScene`. `Controls Hint Canvas` is a non-interactive screen-space Canvas at sorting order 90; its 220×220 `Press Tab Hint` Image uses `game-related scenes/Press tab.png`, anchored 24 pixels from the top-right on a 1920×1080 reference layout. Controls/Dialogue/Pause canvases render above it. `Opening Location Overlay` is a 1920×1080-responsive screen-space Canvas at sorting order 9000. It dims the game to 55% black and centers a 900×271-aspect `game-related scenes/nanjingText.jpeg` title card for 3 seconds, then fades it for 0.35 seconds while gameplay input is suspended. Root transforms now match `background.unity`: Soldier `(22.87, 10.51)`, Silk Seller `(15.61, 19.01)`, Trader `(45.94, 22.98)`, `guopinyuMain_0` `(52.81, 8.66)`, `motherMain_0` `(28.2, -1.98)`, and `lilguyMain_0` `(26.87, -2.43)`. The `PauseController` root also carries `ControlsOverlayController`, which toggles `ControlsScene` additively with Tab. `Player` keeps movement, collision, `PlayerWalkAnimation`, and `OpeningDialogue` on the root, with a `Player Visual` child containing the SpriteRenderer; this lets the visual flip horizontally and bounce without moving the collider. On the first gameplay arrival, `OpeningDialogue` waits for the Nanjing title card to finish and then presents the one-line player monologue “Where am I?” using `CharacterBackground/guoxiaoDialogue.jpeg` and `character_img/xiaoyueConfused.PNG`; `GameProgress` prevents the complete opening sequence from replaying after tracing reloads. NPC `InterractE` prompts preserve the source aspect within `1.4×1.45` world units and leave a `0.12`-unit clear gap above the rendered head; the legacy text prompt is fallback-only. Its single wired NPC `Conversation` component maps `player` to `CharacterBackground/guoxiaoDialogue.jpeg` and `soldier` to `CharacterBackground/soldierDialogue.jpeg`, plus expression-keyed portraits (`normal`, `confused`, `excited`, `worried`, `surprised`, `stern`) from `character_img/`. It also wires `gamevoiceDialogue.jpeg` for narrator/instruction prompts and `multichoiceDialogue.jpeg` for answers. `Dialogue` builds 700×211-aspect frames along the bottom: the active speaker or Game Voice prompt on the left and, on choice steps, a separate Multiple Choice frame on the right. The Silk Seller now owns the 王喜悦 exchange: a direct 我 fill-in tracing handoff, normal/panic/excited portrait changes, an exact 知→道 tracing handoff before 郭小月 asks where her grandmother is, and a final 别→人 tracing handoff after the bridge warning. Only the Silk Seller may claim these saved resume points; later E interactions show a short bridge reminder instead of replaying the lesson.
  ...
  - **Trader interaction:** `Trader` owns the bridge exchange with 吴叔杨. The first conversation uses Xiaoyue's tired/normal portraits and Trader's stern/shock portraits, sends the player through the exact 可→以 tracing task, then resumes at “可以 (Kěyǐ)，谢谢 (xièxie).” Trader has `resumeAfterTracing` enabled with owner key `Trader`, so only this NPC can claim that saved step. Later E interactions use the one-line reminder “Can you please give this book to her?” instead of replaying the full exchange. The Silk Seller's repeat reminder now directs the player to 吴叔杨 waiting at the bridge.
  - **Dialogue input and glyph coverage:** the Silk Seller's first 我 fill-in is a one-option writing choice, so number-pad `1` launches tracing and resumes at step 3. `Dialogue.MakeText` enables multi-atlas expansion on the dynamic Chinese TMP font before creating runtime text; dialogue can add 可, 呀, 怎, and later Chinese glyphs without falling back to square placeholders when its first 1024×1024 atlas fills.
  - **Soldier repeat interaction:** the Soldier's empty main `steps` list continues to use the full authored Xiaoyue conversation only on the first interaction. After that conversation closes, the E prompt reappears; later E presses use the serialized one-line `repeatSteps` exchange “Did you find 王喜悦？” with `soldierDialogue.jpeg` and the normal Soldier portrait, then close without replaying the original sequence. `GameProgress` stores completed NPC keys for the current play session, so this repeat state survives every Weaver/tracing reload of `game1`.
  - **Portrait placement:** each serialized `SpeakerDialoguePortrait` may use a normalized `slotOffset` without changing the source bitmap. Xiaoyue's referenced `xiaoyueExcited_0` sprite slice spans the body and both exclamation-mark pieces (`x=60`, width `402`), preventing the punctuation from being clipped in dialogue.
  - **Latest background resync:** `game1` was rebuilt again from all authoritative background roots. All 122 `BoxCollider2D` components now have the same fileIDs and byte-identical serialized settings as `background.unity`, including colliders on preserved gameplay NPC roots and `guopinyuMain_0`. All non-gameplay root contents and transforms also match the source, including the lower tree row, `Water Tiles_4 (8)`, the furniture table at `(13.43, 18.09)`, tilemap material, and current NPC transforms. The 23 imported flower-cluster SpriteRenderers continue to use `WalkInFront`; gameplay-only Player/NPC controllers, conversations, persistent repeat-state handling, and both UI overlays remain intact.
...
- **`background.unity`** — the authoritative 189-root map source synchronized into `game1`; it includes the newest expanded environment, Trader/Guopinyu content, furniture, flower clusters, large grass-patch roots, and the new `motherMain_0` and `lilguyMain_0` NPC visuals.
- **`hanzi tracing base.unity`** — seven roots: `Main Camera`, `Global Light 2D`, `TraceManager`, `GuideStroke`, `UserStroke`, `StrokeArrow`, and `Tracing Background`. `Tracing Background` is a SpriteRenderer using `game-related scenes/tracingBG.png` (`tracingBG_0`, guid `8d31ff95d1419e24a9a208e617b58ddb`) at world `(0, 0, 1)` and SortingOrder `-100`. Its `CameraFillSprite` component is wired to `Main Camera` and independently fits the sprite's X/Y scale to the live orthographic viewport, eliminating clear-color bars at non-16:9 Game view sizes without changing tracing coordinates. `TraceManager.userStrokeColor` is Inspector-configured as black; correct/incorrect release feedback remains green/red. `availableCharacters` is ordered 不, 要, 我, 知, 道, 别, 人, 可, 以: legacy count-only tasks still consume the initial entries, while newer dialogue tasks select exact `CharacterData.characterName` values in their authored order.
  - `Assets/Character vectors/` contains `CharacterData` assets for 不 (4 strokes), 人 (2), 以 (4), 别 (7), 可 (5), 问 (6), 我 (7), 要 (9), 知 (8), and 道 (12). Every guide polyline uses the full ordered Make Me a Hanzi/Hanzi Writer median data, normalized into the tracing scene's approximately 4×3 coordinate area with `x = (sourceX - 512) / 200` and `y = (sourceY - 387) / 280`.
